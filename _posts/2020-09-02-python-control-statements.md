...
이와 같이 프로그래밍에서도 선택을 해야 하는 순간이 있습니다.

제어문으로 `if`문, `for`문, `while`문이 있습니다.
